...
const SongPlayer: React.FC<SongPlayerProps> = ({ songs }) => {
  ...
  const volumeCapture = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (audioRef.current) {
  ...
  const previousSong = () => {
    ...
    const currentIndex = songs.findIndex((item) => item.song === currentSong.song);
    if (currentIndex > 0) {
      const object = songs[currentIndex - 1];
      dispatch(
        addToStorage({
